sap.ui.define([
		...
		function extendMetadataUrlParameters (aUrlParametersToAdd, oMetadataUrlParams, sServiceUrl) {
			...
			aUrlParametersToAdd.forEach(function (sUrlParam) {
				...
				if (sUrlParam === "sap-language") {
					...
					oMetadataUrlParams["sap-language"]  = sap.ui.getCore().getConfiguration().getLanguage();
				} else {
		...
		return {
			...
			createODataModel: function (oOptions) {
				...
				if (!oOptions.url) {
					jQuery.sap.log.error("Please provide a url when you want to create an ODataModel", "pg/kanban.model.models.createODataModel");
					return null;
				}
